  ...
  return (
    ...
    >
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={0.8}>
        <Typography
          sx={{ fontSize: 10, fontWeight: 800, color: C.n500, textTransform: "uppercase" }}
        >
          {label}
        </Typography>
...
function AiCenter({ prospects, tasks, opportunities }) {
  ...
  const qualifiedAi = aiProspects.filter(
    (p) => p.status === "qualified" || getProspectScore(p) >= 70
  );
  const messagesReady = prospects.filter((p) =>
    ["message_ready", "prepared", "ready_to_send"].includes(p.engagement_status || p.status)
  );
  const replied = prospects.filter((p) =>
    ["replied", "reply_detected", "follow_up_required"].includes(p.engagement_status || p.status)
  );
  const hotProspects = prospects.filter((p) => getProspectScore(p) >= 80).length;
  const qualificationRate = aiProspects.length
    ? Math.round((qualifiedAi.length / aiProspects.length) * 100)
    : 0;
      ...
      <Grid container spacing={1.5} mb={2.5}>
        ...
        <Grid item xs={6} md={3}>
          <MiniKpi
            ...
            color={C.green}
            sub={`${qualifiedAi.length} qualifiés`}
          />
        </Grid>
        <Grid item xs={6} md={3}>
          <MiniKpi
            ...
            {[
              {
                ...
                status: aiProspects.length > 0 ? "Actif" : "En attente",
                color: aiProspects.length > 0 ? C.green : C.amber,
              },
              {
                label: "Agent Engagement",
                status: messagesReady.length || replied.length ? "Actif" : "En attente",
                color: messagesReady.length || replied.length ? C.green : C.amber,
              },
              {
                label: "Google Maps",
                status: process.env.REACT_APP_GOOGLE_MAPS_API_KEY ? "Connecté" : "Clé manquante",
                color: process.env.REACT_APP_GOOGLE_MAPS_API_KEY ? C.green : C.red,
              },
    ...
    return (
      <Box>
        <Alert severity="info" sx={{ borderRadius: 2, mb: 2, fontSize: 13 }}>
          Aucun prospect avec latitude/longitude. La carte Google Maps sera affichée dès que les
          coordonnées existent dans le backend.
        </Alert>
        <CityFallback cities={fallbackByCity} />
      </Box>
    );
    ...
    <Box>
          ...
          {selected && (
            ...
            >
              <Box sx={{ minWidth: 180 }}>
                ...
                <Typography sx={{ fontSize: 12, color: C.n600 }}>
                  {selected.city || selected.address || selected.country || "—"}
                </Typography>
                <Typography sx={{ fontSize: 12, color: C.n600 }}>
                  Source : {getSource(selected)}
                </Typography>
                <Typography sx={{ fontSize: 12, color: C.n600 }}>
                  Score IA : {getProspectScore(selected) || "—"}
                </Typography>
              </Box>
            </InfoWindow>
          )}
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" mt={1.5}>
        <Typography sx={{ fontSize: 11, color: C.n500 }}>
          {mappedProspects.length} prospects affichés sur Google Maps
        </Typography>
        <Typography sx={{ fontSize: 11, color: C.n400 }}>
          {prospects.length - mappedProspects.length} sans coordonnées
        </Typography>
      </Stack>
    </Box>
  );
...
function CityFallback({ cities }) {
  if (!cities.length)
    return (
      <Typography sx={{ fontSize: 13, color: C.n400 }}>Aucune donnée géographique.</Typography>
    );
        ...
        <Box key={c.city}>
          ...
          <LinearProgress
            ...
            sx={{
              ...
              borderRadius: 3,
              bgcolor: C.n100,
              "& .MuiLinearProgress-bar": { bgcolor: C.blue, borderRadius: 3 },
            }}
          />
        </Box>
      ))}
  ...
  const data = stages.map((s) => {
    ...
    return {
      ...
      forecast: items.reduce(
        (sum, o) => sum + Number(o.amount || 0) * (Number(o.probability || 0) || s.probability),
        0
      ),
    };
  });
  ...
  return (
    <AccentCard accent={C.green}>
      <SectionTitle
        ...
        title="Pipeline & Prévision CA"
        subtitle="Prévision basée sur le stade et la probabilité"
      />
      <Grid container spacing={2}>
        <Grid item xs={12} md={7}>
          {opportunities.length ? (
            <ResponsiveContainer width="100%" height={250}>
              <FunnelChart>
                <ReTooltip formatter={(value, name) => [`${value} opportunité(s)`, name]} />
        ...
        <Grid item xs={12} md={5}>
          <Stack spacing={1.3}>
            ...
            <MiniKpi
              label="Prévision CA"
              value={fmtTND(forecast)}
              icon={<TrendingUp fontSize="small" />}
              color={C.blue}
            />
            <Box sx={{ p: 1.4, borderRadius: 12, bgcolor: C.n50, border: `1px solid ${C.n200}` }}>
              <Typography sx={{ fontSize: 11, fontWeight: 800, color: C.n500, mb: 1 }}>
                Détail par étape
              </Typography>
              <Stack spacing={0.8}>
                {data.map((d) => (
                  ...
                  >
                    ...
                    <Typography sx={{ fontSize: 11, fontWeight: 800, color: d.fill }}>
                      {d.value}
                    </Typography>
                  </Stack>
                ))}
...
function SmartAlerts({ prospects, tasks, opportunities, members }) {
  ...
  prospects.forEach((p) => {
    ...
    if (score >= 80 && !["contacted", "message_ready", "replied", "won"].includes(status)) {
      alerts.push({
        type: "warning",
        title: `Prospect chaud sans suivi : ${getProspectName(p)}`,
        sub: `Score IA ${score}% · action recommandée : relance rapide`,
      });
    }
    if (["replied", "reply_detected", "follow_up_required"].includes(status)) {
      alerts.push({
        type: "info",
        title: `Réponse détectée : ${getProspectName(p)}`,
        sub: "Un commercial doit traiter la conversation",
      });
    }
  });
  ...
  opportunities.forEach((o) => {
    ...
    if (!["won", "lost"].includes(o.stage || o.status) && days >= 7) {
      alerts.push({
        type: "warning",
        title: `Opportunité inactive : ${o.name || o.title || "Opportunité"}`,
        sub: `${days} jours sans activité · relance recommandée`,
      });
    }
  });
  ...
  tasks.filter(isOverdue).forEach((t) => {
    alerts.push({
      ...
      sub: `${t.assigned_to_detail?.username || "Commercial"} · échéance dépassée`,
    });
  });
  ...
  members.forEach((m) => {
    ...
    if (memberTasks.length >= 3 && done === 0) {
      alerts.push({
        type: "warning",
        title: `Activité faible : ${m.username}`,
        sub: "Aucune tâche terminée récemment · coaching conseillé",
      });
    }
  });
  ...
  return (
    <AccentCard accent={C.amber}>
      <SectionTitle
        ...
        title="Alertes IA Manager"
        subtitle="Risques et actions à suivre"
      />
      {alerts.length ? (
...
function ManagerRecommendations({ prospects, tasks, opportunities, leaderboard }) {
  ...
  const replied = prospects.find((p) =>
    ["replied", "reply_detected", "follow_up_required"].includes(p.engagement_status || p.status)
  );
  ...
  if (top)
    recs.push({
      icon: "🏆",
      title: `${top.username} est le meilleur performer`,
      action: "Prévoir une reconnaissance ou un bonus",
      color: C.green,
    });
  if (low)
    recs.push({
      icon: "🎯",
      title: `${low.username} a un score faible`,
      action: "Planifier un coaching individuel",
      color: C.red,
    });
  if (hot)
    recs.push({
      icon: "🔥",
      title: `Prospect chaud : ${getProspectName(hot)}`,
      action: "Assigner une relance prioritaire",
      color: C.amber,
    });
  if (replied)
    recs.push({
      icon: "💬",
      title: `Réponse à traiter : ${getProspectName(replied)}`,
      action: "Demander au commercial de répondre aujourd'hui",
      color: C.blue,
    });
  if (riskyOpp)
    recs.push({
      icon: "📈",
      title: `Opportunité à suivre : ${riskyOpp.name || riskyOpp.title || "Opportunité"}`,
      action: "Vérifier la prochaine action commerciale",
      color: C.purple,
    });
  ...
  if (!recs.length) {
    recs.push({
      ...
      action: "Continuer le suivi hebdomadaire de l'équipe",
      color: C.green,
    });
  }
  ...
  return (
    <AccentCard accent={C.indigo}>
      <SectionTitle
        ...
        title="Recommandations Manager"
        subtitle="Actions intelligentes proposées"
      />
...
ManagerRecommendations.propTypes = {
  prospects: PropTypes.array,
  tasks: PropTypes.array,
  opportunities: PropTypes.array,
  leaderboard: PropTypes.array,
};
ManagerRecommendations.defaultProps = {
  prospects: [],
  tasks: [],
  opportunities: [],
  leaderboard: [],
              ...
              >
                ...
                <TableCell sx={{ py: 1.5 }}>
                  <Stack direction="row" alignItems="center" spacing={0.5}>
                    <Typography sx={{ fontSize: 17, fontWeight: 900, color }}>
                ...
                <TableCell sx={{ py: 1.5, minWidth: 100 }}>
                  <Typography sx={{ fontSize: 10, color: C.n500, mb: 0.3 }}>
                  ...
                  <LinearProgress
                    ...
                    sx={{
                      ...
                      borderRadius: 2,
                      bgcolor: C.n100,
                      "& .MuiLinearProgress-bar": { bgcolor: C.blue, borderRadius: 2 },
                    }}
                  />
                </TableCell>
                <TableCell sx={{ py: 1.5 }}>
                  <Stack direction="row" spacing={0.5}>
                    {[
                      { icon: <Phone />, val: entry.calls || 0, color: C.blue },
                      { icon: <Email />, val: entry.emails || 0, color: C.purple },
                      { icon: <Groups />, val: entry.meetings || 0, color: C.teal },
                    ].map((it, i) => (
                      <Chip
                        key={i}
                        size="small"
                        icon={React.cloneElement(it.icon, { sx: { fontSize: 12 } })}
                        label={it.val}
                        sx={{
                          height: 22,
                          fontSize: 10,
                ...
                <TableCell sx={{ py: 1.5, fontSize: 13, fontWeight: 900, color: C.teal }}>
...
function TeamPerformanceTab({ members, prospects, tasks, opportunities }) {
  ...
  const avgScore =
    ...
    (leaderboardList.length
      ? Math.round(
          leaderboardList.reduce((s, e) => s + Number(e.score || 0), 0) / leaderboardList.length
        )
      : 0);
  ...
  return (
    <Box sx={{ animation: `${fadeUp} 0.2s ease` }}>
      <Grid container spacing={2} mb={3}>
        <Grid item xs={6} sm={3}>
          <KpiTile color={scoreColor(avgScore)}>
            <ScoreRing score={avgScore} size={48} />
            <Box>
              <Typography sx={{ fontSize: 10, color: C.n400, fontWeight: 800 }}>
                Score moyen
              </Typography>
              <Typography sx={{ fontSize: 20, fontWeight: 900, color: scoreColor(avgScore) }}>
                {Math.round(avgScore)}%
              </Typography>
            </Box>
          </KpiTile>
        ...
        <Grid item xs={6} sm={3}>
          <KpiTile color={C.blue}>
            <People sx={{ color: C.blue }} />
            <Box>
              <Typography sx={{ fontSize: 10, color: C.n400, fontWeight: 800 }}>
                Commerciaux
              </Typography>
              <Typography sx={{ fontSize: 20, fontWeight: 900, color: C.blue }}>
                {teamData?.team_count || members.length}
              </Typography>
            </Box>
          </KpiTile>
        </Grid>
            ...
            <Box>
              ...
              <Typography sx={{ fontSize: 20, fontWeight: 900, color: C.red }}>
                {(teamData?.in_difficulty || []).length ||
                  leaderboardList.filter((e) => e.score < 60).length}
              </Typography>
            </Box>
          </KpiTile>
        </Grid>
        <Grid item xs={6} sm={3}>
          <KpiTile color={C.amber}>
            <Timeline sx={{ color: C.amber }} />
            <Box>
              <Typography sx={{ fontSize: 10, color: C.n400, fontWeight: 800 }}>
                Alertes KPI
              </Typography>
      ...
      {externalAlerts.length > 0 && (
        <Box mb={3}>
          <Grid container spacing={1}>
            {externalAlerts.slice(0, 4).map((a, i) => (
        ...
        <Grid item xs={12} lg={8}>
          <AccentCard accent={C.red}>
            <SectionTitle
              ...
              subtitle="Performance de l'équipe ce mois"
            />
            <PerformanceTable
  ...
  const body = (
    <Box sx={{ animation: `${fadeUp} 0.2s ease` }}>
      <DashboardCommandCenter
        eyebrow="Cockpit Manager IA"
        title={`Bonjour, ${user.username || "Manager"}`}
        subtitle="Pilotage équipe, agents IA, pipeline et géographie commerciale"
        ...
        alerts={[
          ...(overdueTasks.length
            ...
            : []),
          { label: `${members.length} membres`, icon: <People /> },
        ]}
        metrics={[
          {
            label: "Prospects",
          ...
          },
          {
            ...
            helper: `${overdueTasks.length} en retard`,
            icon: <CheckCircle />,
            tone: C.amber,
          },
          {
      ...
      {activeTab === 0 && (
        <Box sx={{ animation: `${fadeUp} 0.2s ease` }}>
          <Grid container spacing={2.5} mb={3}>
            <Grid item xs={12} sm={6} md={3}>
              <KpiTile color={C.blue}>
                ...
                <Box>
                  ...
                  <Typography sx={{ fontSize: 24, fontWeight: 900, color: C.blue }}>
                    {members.length}
                  </Typography>
                </Box>
              </KpiTile>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <KpiTile color={C.purple}>
                <PersonAdd sx={{ color: C.purple }} />
                <Box>
                  <Typography sx={{ fontSize: 10, color: C.n400, fontWeight: 800 }}>
                    Prospects équipe
                  </Typography>
                  <Typography sx={{ fontSize: 24, fontWeight: 900, color: C.purple }}>
                    {prospects.length}
                  </Typography>
                  <Typography sx={{ fontSize: 11, color: C.n500 }}>
                    {hotProspects.length} chauds
                  </Typography>
                </Box>
              </KpiTile>
            </Grid>
            <Grid item xs={12} sm={6} md={3}>
              <KpiTile color={C.amber}>
                <CheckCircle sx={{ color: C.amber }} />
                <Box>
                  <Typography sx={{ fontSize: 10, color: C.n400, fontWeight: 800 }}>
                    Tâches actives
                  </Typography>
                  <Typography sx={{ fontSize: 24, fontWeight: 900, color: C.amber }}>
                    {activeTasks.length}
                  </Typography>
                  <Typography sx={{ fontSize: 11, color: C.n500 }}>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <KpiTile color={C.green}>
                <AttachMoney sx={{ color: C.green }} />
                <Box>
                  <Typography sx={{ fontSize: 10, color: C.n400, fontWeight: 800 }}>
                    Pipeline équipe
                  </Typography>
                  <Typography sx={{ fontSize: 24, fontWeight: 900, color: C.green }}>
                    {fmtTND(pipeline)}
                  </Typography>
                  <Typography sx={{ fontSize: 11, color: C.n500 }}>
                    {wonOpps.length} gagnées
                  </Typography>
                </Box>
              </KpiTile>
            </Grid>
          ...
          <Grid container spacing={2.5} mb={2.5}>
            <Grid item xs={12} lg={8}>
              <AiCenter prospects={prospects} tasks={tasks} opportunities={opportunities} />
            </Grid>
            <Grid item xs={12} lg={4}>
            ...
            <Grid item xs={12} lg={7}>
              <AccentCard accent={C.blue}>
                <SectionTitle
                  ...
                  subtitle="Prospects et tâches terminées par commercial"
                />
                {barData.length ? (
            ...
            <Grid item xs={12} lg={5}>
              <AccentCard accent={C.teal}>
                <SectionTitle
                  ...
                  subtitle="Prospects et opportunités"
                />
                <ResponsiveContainer width="100%" height={240}>
            ...
            <Grid item xs={12} lg={6}>
              <AccentCard accent={C.blue}>
                <SectionTitle
                  ...
                  subtitle="Google Maps des prospects de l'équipe"
                />
          ...
          <AccentCard accent={C.purple}>
            <SectionTitle
              ...
              subtitle="Suivi rapide des commerciaux"
            />
            <Grid container spacing={1.5}>
              {members.map((m) => {
                    ...
                    >
                      ...
                      <LinearProgress
                        ...
                        sx={{
                          ...
                          borderRadius: 3,
                          bgcolor: C.n100,
                          "& .MuiLinearProgress-bar": { bgcolor: C.green, borderRadius: 3 },
                        }}
                      />
                    </Box>
                  </Grid>
