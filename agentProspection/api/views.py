...
class ProspectScoreView(APIView):
    permission_classes = [
        IsAuthenticated
    ]
    ...
    def post(
        ...
        previous_score = (
            prospect.prospect_company.score_ia
        ...
        # Le Prospect garde l'évaluation
        # et la justification.
        prospect.evaluation = (
            evaluation
        )
        ...
        # updated_at seulement si le modèle possède ce champ.
        if hasattr(
            prospect,
            "updated_at",
        ):
        ...
        if prospect.prospect_company:
            ...
            prospect_company.evaluation = (
                evaluation
            )

            prospect_company.save(
                update_fields=[
                    "score_ia",
                    "evaluation",
                ]
        ...
        try:
            ...
            from sales.models import ProspectScoreHistory
            from sales.prospect_tracking_service import record_agent_run, record_prospect_activity
            ...
            agent_run = record_agent_run(
                ...
                input_summary="Recalcul manuel du score prospect",
                output_summary="; ".join(map(str, reasons)),
                metadata={"score": result.get("score", 0), "evaluation": evaluation},
            )
            score_history = ProspectScoreHistory.objects.create(
                prospect=prospect,
                previous_score=previous_score or 0,
                new_score=result.get("score", 0) or 0,
                reason="Qualification terminée",
                agent_run=agent_run,
            )
            event_from_score_history(score_history)
            record_prospect_activity(
                prospect=prospect,
                activity_type="interaction_detected",
                title="Qualification IA terminée",
                description="; ".join(map(str, reasons)),
                source="agent",
                agent_run=agent_run,
                metadata={"score": result.get("score", 0), "evaluation": evaluation},
            ...
            import logging
            logging.getLogger(__name__).exception("Prospect 360 qualification tracking failed")
